...
//
package fuse // import "memfs/fuse"
...
func (c *Conn) ReadRequest() (Request, error) {
	m := getMessage(c)
	log.Printf("ReadRequest|getMessage|m.hdr:%+v", m.hdr)
	c.rio.RLock()
	...
	switch m.hdr.Opcode {
	default:
		log.Printf("switch Opcode|curOpCode:%d|default", m.hdr.Opcode)
		Debug(noOpcode{Opcode: m.hdr.Opcode})
		goto unrecognized

	case opLookup:
		log.Printf("switch Opcode|curOpCode:%d|oplookup|header:%+v", m.hdr.Opcode, m.Header())
		buf := m.bytes()
		n := len(buf)
		if n == 0 || buf[n-1] != '\x00' {
			goto corrupt
	...
	case opForget:
		log.Printf("switch Opcode|curOpCode:%d|opForget|header:%+v", m.hdr.Opcode, m.Header())
		in := (*forgetIn)(m.data())
	...
	case opGetattr:
		log.Printf("switch Opcode|curOpCode:%d|opGetattr|header:%+v", m.hdr.Opcode, m.Header())
		switch {
	...
	case opSetattr:
		log.Printf("switch Opcode|curOpCode:%d|opSetattr|header:%+v", m.hdr.Opcode, m.Header())
		in := (*setattrIn)(m.data())
		if m.len() < unsafe.Sizeof(*in) {
			goto corrupt
		}
	...
	case opReadlink:
		log.Printf("switch Opcode|curOpCode:%d|opReadlink|header:%+v", m.hdr.Opcode, m.Header())
		if len(m.bytes()) > 0 {
			goto corrupt
		}
		req = &ReadlinkRequest{
	...
	case opSymlink:
		log.Printf("switch Opcode|curOpCode:%d|opSymlink|header:%+v", m.hdr.Opcode, m.Header())
	...
	case opLink:
		log.Printf("switch Opcode|curOpCode:%d|oplink|header:%+v", m.hdr.Opcode, m.Header())
		in := (*linkIn)(m.data())
		if m.len() < unsafe.Sizeof(*in) {
			goto corrupt
		}
	...
	case opMknod:
		log.Printf("switch Opcode|curOpCode:%d|opMKnod|header:%+v", m.hdr.Opcode, m.Header())
	...
	case opMkdir:
		log.Printf("switch Opcode|curOpCode:%d|opMkdir|header:%+v", m.hdr.Opcode, m.Header())
	...
	case opUnlink, opRmdir:
		log.Printf("switch Opcode|curOpCode:%d|opUnlink, opRmdir|header:%+v", m.hdr.Opcode, m.Header())
		buf := m.bytes()
		n := len(buf)
		if n == 0 || buf[n-1] != '\x00' {
			goto corrupt
	...
	case opRename:
		log.Printf("switch Opcode|curOpCode:%d|opRename|header:%+v", m.hdr.Opcode, m.Header())
		in := (*renameIn)(m.data())
		if m.len() < unsafe.Sizeof(*in) {
			goto corrupt
		}
	...
	case opOpendir, opOpen:
		log.Printf("switch Opcode|curOpCode:%d|opOpendir,opOpen|header:%+v", m.hdr.Opcode, m.Header())
		in := (*openIn)(m.data())
		if m.len() < unsafe.Sizeof(*in) {
			goto corrupt
		}
	...
	case opRead, opReaddir:
		log.Printf("switch Opcode|curOpCode:%d|opRead,opReaddir|header:%+v", m.hdr.Opcode, m.Header())
		in := (*readIn)(m.data())
	...
	case opWrite:
		log.Printf("switch Opcode|curOpCode:%d|opWrite|header:%+v", m.hdr.Opcode, m.Header())
		in := (*writeIn)(m.data())
	...
	case opStatfs:
		log.Printf("switch Opcode|curOpCode:%d|opStatfs|header:%+v", m.hdr.Opcode, m.Header())
		req = &StatfsRequest{
			Header: m.Header(),
		}

	case opRelease, opReleasedir:
		log.Printf("switch Opcode|curOpCode:%d|opRelease|header:%+v", m.hdr.Opcode, m.Header())
		in := (*releaseIn)(m.data())
		if m.len() < unsafe.Sizeof(*in) {
			goto corrupt
		}
	...
	case opFsync, opFsyncdir:
		log.Printf("switch Opcode|curOpCode:%d|opFsync|header:%+v", m.hdr.Opcode, m.Header())
		in := (*fsyncIn)(m.data())
		if m.len() < unsafe.Sizeof(*in) {
			goto corrupt
		}
	...
	case opSetxattr:
		log.Printf("switch Opcode|curOpCode:%d|opSetxattr|header:%+v", m.hdr.Opcode, m.Header())
	...
	case opGetxattr:
		log.Printf("switch Opcode|curOpCode:%d|opGetxattr|header:%+v", m.hdr.Opcode, m.Header())
	...
	case opListxattr:
		log.Printf("switch Opcode|curOpCode:%d|opListxattr|header:%+v", m.hdr.Opcode, m.Header())
		in := (*getxattrIn)(m.data())
		if m.len() < unsafe.Sizeof(*in) {
			goto corrupt
		}
	...
	case opRemovexattr:
		log.Printf("switch Opcode|curOpCode:%d|opRemovexattr|header:%+v", m.hdr.Opcode, m.Header())
		buf := m.bytes()
		n := len(buf)
		if n == 0 || buf[n-1] != '\x00' {
			goto corrupt
	...
	case opFlush:
		log.Printf("switch Opcode|curOpCode:%d|opFlush|header:%+v", m.hdr.Opcode, m.Header())
		in := (*flushIn)(m.data())
		if m.len() < unsafe.Sizeof(*in) {
			goto corrupt
		}
	...
	case opInit:
		log.Printf("switch Opcode|curOpCode:%d|opInit|header:%+v", m.hdr.Opcode, m.Header())
		in := (*initIn)(m.data())
		if m.len() < unsafe.Sizeof(*in) {
			goto corrupt
		}
	...
	case opAccess:
		log.Printf("switch Opcode|curOpCode:%d|opAccess|header:%+v", m.hdr.Opcode, m.Header())
		in := (*accessIn)(m.data())
		if m.len() < unsafe.Sizeof(*in) {
			goto corrupt
		}
	...
	case opCreate:
		log.Printf("switch Opcode|curOpCode:%d|opCreate|header:%+v", m.hdr.Opcode, m.Header())
	...
	case opInterrupt:
		log.Printf("switch Opcode|curOpCode:%d|opInterrupt|header:%+v", m.hdr.Opcode, m.Header())
	...
	case opDestroy:
		log.Printf("switch Opcode|curOpCode:%d|opDestory|header:%+v", m.hdr.Opcode, m.Header())
		req = &DestroyRequest{
			Header: m.Header(),
		}

	case opNotifyReply:
		log.Printf("switch Opcode|curOpCode:%d|opNotifyReply|header:%+v", m.hdr.Opcode, m.Header())
		req = &NotifyReply{
			Header: m.Header(),
			msg:    m,
		}
	...
	case opPoll:
		log.Printf("switch Opcode|curOpCode:%d|opPoll|header:%+v", m.hdr.Opcode, m.Header())
		in := (*pollIn)(m.data())
		if m.len() < unsafe.Sizeof(*in) {
			goto corrupt
		}
	...
	case opBatchForget:
		log.Printf("switch Opcode|curOpCode:%d|opBatchForget|header:%+v", m.hdr.Opcode, m.Header())
	...
	case opSetlk, opSetlkw:
		log.Printf("switch Opcode|curOpCode:%d|opSetlk,opSetlkw|header:%+v", m.hdr.Opcode, m.Header())
		in := (*lkIn)(m.data())
		if m.len() < unsafe.Sizeof(*in) {
			goto corrupt
		}
	...
	case opGetlk:
		log.Printf("switch Opcode|curOpCode:%d|opGetlk|header:%+v", m.hdr.Opcode, m.Header())
		in := (*lkIn)(m.data())
		if m.len() < unsafe.Sizeof(*in) {
			goto corrupt
		}
